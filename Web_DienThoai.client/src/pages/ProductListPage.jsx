  ...
  useEffect(() => {
    ...
    productApi
      ...
      .then((res) => {
        ...
        if (filters.q) {
          data = data.filter((p) => p.name.toLowerCase().includes(filters.q.toLowerCase()));
        }

        if (filters.categorySlug) {
          data = data.filter((p) => p.category?.slug === filters.categorySlug);
        }
        ...
        const mapped = data.map((p) => ({
          ...p,
          mainImageUrl: resolveImage(p.images?.[0] || p.mainImageUrl),
        }));
    ...
    () =>
      [
        ...
        filters.minPrice !== undefined || filters.maxPrice !== undefined
          ? `Giá: ${filters.minPrice ? filters.minPrice.toLocaleString("vi-VN") : "0"} - ${filters.maxPrice ? filters.maxPrice.toLocaleString("vi-VN") : "8"}`
          : null,
      ].filter(Boolean),
    [filters],
  );
            ...
            {vouchers.slice(0, 4).map((voucher) => (
              ...
              >
                ...
                {voucher.maxDiscountAmount ? (
                  <span className="text-slate-500">max {formatVnd(voucher.maxDiscountAmount)}</span>
                ) : null}
              </button>
            ))}
          </div>
      ...
      <div className="flex gap-6">
        <aside
          className={`
            ...
            w-full md:static md:z-auto md:block md:w-64 md:bg-transparent md:p-0
          `}
        >
          {filterOpen && (
